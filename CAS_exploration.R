...
# 4.1 Create a table to lay out the differences more easily
...
# 4.2 and 4.3 Where are the mismatches the most common?  
...
# Issue 5 Mismatch: PSC fishery ID
...
# 5.2 Where are the mismatches most common?
...
compare_CAS_MRP_summary  <- compare_CAS_MRP_summary   %>% 
  ...
  add_row(Issue_category= "Missing", Issue_ID="2", Issue="Recovery id by year in dfo_CAS not MRP", Count=nrow(dfo_CAS_not_MRP_recid), Definition="Recovery Id and run year combinations that are in the DFO recoveries in the CAS database but not in MRP database ") %>% 
  add_row(Issue_category= "Missing", Issue_ID="2.1", Issue="Recovery id by year in dfo_CAS not MRP - fishery", Count=nrow(dfo_CAS_not_MRP_recid_Fishery), Definition="Recovery Id and run year combinations that are in the DFO recoveries in the CAS database but not in MRP database, summarized by Fishery ") %>% 
  add_row(Issue_category= "Missing", Issue_ID="2.2", Issue="Recovery id by year in dfo_CAS not MRP - recovery site", Count=nrow(dfo_CAS_not_MRP_recid_RecoverySite), Definition="Recovery Id and run year combinations that are in the DFO recoveries in the CAS database but not in MRP database, summarized by recovery site ") %>% 
  add_row(Issue_category= "Altered", Issue_ID="3", Issue="CWT estimate mismatch value MRP and CAS", Count=nrow(mrp_CAS_estimate_mismatch), Definition="For a given pair of recovery Id and year, the CWT estimate in MRP is not equal to AdjustedEstimatedNumber in CAS") %>% 
  add_row(Issue_category= "Altered", Issue_ID="3.1", Issue="CWT estimate mismatch value MRP and CAS - comparison table", Count=nrow(mrp_CAS_estimate_mis), Definition="Smaller table with CWT estimate in MRP and AdjustedEstimatedNumber in CAS, sorted by the absolute difference between the two values") %>% 
  add_row(Issue_category= "Altered", Issue_ID="3.2", Issue="CWT estimate mismatch value MRP and CAS - fishery", Count=nrow(mrp_CAS_estimate_mis_Fishery), Definition="Mismatches in CWT estimate and AdjustedEstimatedNumber summarized by fishery") %>% 
  add_row(Issue_category= "Altered", Issue_ID="3.3", Issue="CWT estimate mismatch value MRP and CAS - location", Count=nrow(mrp_CAS_estimate_mis_RecoverySite), Definition="Mismatches in CWT estimate and AdjustedEstimatedNumber summarized by recovery location") %>% 
  add_row(Issue_category= "Altered", Issue_ID="3.4", Issue="CWT estimate mismatch value MRP and CAS - year", Count=nrow(mrp_CAS_estimate_mis_recovery_year), Definition="Mismatches in CWT estimate and AdjustedEstimatedNumber summarized by recovery year") %>% 
  add_row(Issue_category= "Altered", Issue_ID="4", Issue="Location mismatch value MRP and CAS", Count=nrow(mrp_CAS_location_mismatch), Definition="Mismatches in rec_location_code in MRP and RecoverySite in CAS") %>% 
  add_row(Issue_category= "Altered", Issue_ID="4.1", Issue="Location mismatch value MRP and CAS - comparison table", Count=nrow(mrp_CAS_location_mismatch), Definition="Mismatches in rec_location_code in MRP and RecoverySite in CAS, comparison table") %>% 
  add_row(Issue_category= "Altered", Issue_ID="4.2", Issue="Location mismatch value MRP and CAS - fishery", Count=nrow(mrp_CAS_location_mis_Fishery), Definition="Mismatches in rec_location_code in MRP and RecoverySite in CAS, summarized by fishery") %>% 
  add_row(Issue_category= "Altered", Issue_ID="4.3", Issue="Location mismatch value MRP and CAS - year", Count=nrow(mrp_CAS_location_mis_recovery_year), Definition="Mismatches in rec_location_code in MRP and RecoverySite in CAS, summarized by recovery year") %>% 
...
sheet_list_compare_CAS_MRP<-list(Summary=compare_CAS_MRP_summary,
                                 ...
                                 "2 - RecID_dfoCAS_not_MRP"= dfo_CAS_not_MRP_recid, 
                                 "2.1 - fishery"= dfo_CAS_not_MRP_recid_Fishery,
                                 "2.2 - location"= dfo_CAS_not_MRP_recid_RecoverySite, 
                                 "3 - CWT estimate mismatch"= mrp_CAS_estimate_mismatch, 
                                 "3.1 - differences"= mrp_CAS_estimate_mis, 
                                 "3.2 - fishery"= mrp_CAS_estimate_mis_Fishery, 
                                 "3.3 - location"= mrp_CAS_estimate_mis_RecoverySite, 
                                 "3.4 - year"= mrp_CAS_estimate_mis_recovery_year, 
                                 "4 - Location mismatch" = mrp_CAS_location_mismatch, 
                                 "4.1 - comparison" = mrp_CAS_location_mismatch_trim_join,
                                 "4.2 - fishery" = mrp_CAS_location_mis_Fishery, 
                                 "4.3 - year" = mrp_CAS_location_mis_recovery_year, 
                                 "5 - Psc fishery mismatch" = mrp_CAS_pscfishery_mismatch, 
                                 "5.1 - comparison" = mrp_CAS_pscfishery_mismatch_trim_join, 
                                 "5.2 - fishery" = mrp_CAS_pscfishery_mis_Fishery)
